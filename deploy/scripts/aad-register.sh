...
register_service(){
    ...
    objectId=$(az ad app create --display-name $applicationName --homepage "https://localhost" --query objectId -o tsv)
    objectId=${objectId//[[:space:]]}
    pw=$(< /dev/urandom tr -dc _A-Z-a-z-0-9 | head -c${1:-32})
    tenantId=$(az ad app credential reset --id $objectId --append --password $pw --query tenant -o tsv)
    tenantId=${tenantId//[[:space:]]}
    tenantName=$(az ad app show --id $objectId --query publisherDomain -o tsv)
    tenantName=${tenantName//[[:space:]]}
...
unregister_service(){
    ...
    for id in $(az ad app list --display-name $applicationName --query objectId -o tsv); do
        objectId=${id//[[:space:]]}
