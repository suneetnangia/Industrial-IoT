...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --name)            applicationName="$2"; shift ;;
        --keyvault)        keyVaultName="$2"; shift ;;
        --msi)             keyVaultMsi="$2"; shift ;;
        --subscription)    subscription="$2"; shift ;;
        --clean)           mode="clean" ;;
        --unregister)      mode="unregisteronly" ;;
        --identity)        login="unattended" ;;
        --config)          config="$2"; shift ;;
        --user)            principalId="$2"; shift ;;
        --password)        principalPassword="$2"; shift ;;
        --tenant)          tenantId="$2"; shift ;;
        --audience)        audience="$2"; shift ;;
        --owner)           ownerId="$2"; shift ;;
        --serviceurl)      serviceurl="$2"; shift ;;
        *)                 usage ;;
    esac
    shift
done
