...
while [ "$#" -gt 0 ]; do
    case "$1" in
        --admin)                ADMIN="$2"; shift ;;
        --name)                 DOCKER_COMPOSE_FILE="$2.yml"; shift ;;
        --imagesTag)            IMAGES_TAG="$2"; shift ;;
        --dockerServer)         DOCKER_SERVER="$2"; shift ;;
        --dockerUser)           DOCKER_USER="$2"; shift ;;
        --dockerPassword)       DOCKER_PASSWORD="$2"; shift ;;
    esac
    shift
done
